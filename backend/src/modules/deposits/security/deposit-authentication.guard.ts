...
export interface DepositPrincipal {
  userId: string;
  tenantId: string;
  role: string;
}

export type DepositRequest = Request & { principal?: DepositPrincipal };

@Injectable()
export class DepositAuthenticationGuard implements CanActivate {
  constructor(
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const request = context.switchToHttp().getRequest<DepositRequest>();
