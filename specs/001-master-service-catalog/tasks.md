...
- [x] T044 [US3] Implement `SnapshotService` to create draft snapshots from curated master services and categories only, publish them with `publishedAt`, and prevent modification after publication — `backend/src/modules/catalog/snapshot/snapshot.service.ts`
- [x] T045 [US3] Implement the protected administrative endpoint for creating and publishing master catalog snapshots — `backend/src/modules/catalog/presentation/snapshot.controller.ts`
- [x] T046 [US3] Implement the export service for published snapshots using only approved public fields; exclude `providerOrigin`, `externalId`, `rawPayload`, credentials, and internal provider metadata — `backend/src/modules/catalog/snapshot/export.service.ts`
- [x] T047 [US3] Implement the protected administrative endpoint for exporting a published master catalog snapshot — `backend/src/modules/catalog/presentation/snapshot-export.controller.ts`
...
- [x] T056 Wire `CatalogAuthorizationGuard` to the platform's real authenticated-user/session and role mechanism from `AuthModule`, removing the unconditional `DevelopmentCatalogAuthorization` binding in `CatalogModule` — `backend/src/modules/catalog/catalog.module.ts`, `backend/src/modules/catalog/security/` per Constitution XI, FR-011 (contradicts)
- [ ] T057 Implement the approved in-process NestJS `@nestjs/schedule` CRON job for scheduled provider synchronization alongside the existing on-demand trigger — `backend/src/modules/catalog/sync/sync.service.ts` per FR-001, plan: Synchronization (v1) (missing)
- [ ] T058 Call `AuditService.recordImport` for each sync job outcome and call `recordSnapshotPublication` / `recordExport` from the snapshot and export flows so import and publish actions produce audit entries — `backend/src/modules/catalog/sync/import-orchestrator.ts`, `backend/src/modules/catalog/snapshot/snapshot.service.ts`, `backend/src/modules/catalog/snapshot/export.service.ts` per FR-009, US1/AC4 (partial)
- [ ] T059 Add URI-based API versioning (enable NestJS versioning and prefix catalog routes with `v1`) on every public catalog controller — `backend/src/main.ts`, `backend/src/modules/catalog/presentation/*.controller.ts` per Constitution IX (missing)
- [ ] T060 Add Swagger/OpenAPI decorators (`@ApiTags`, `@ApiOperation`, `@ApiResponse`, `@ApiBearerAuth`) to all catalog controllers and `@ApiProperty` annotations to all catalog DTOs and enums so generated documentation matches the implementation — `backend/src/modules/catalog/presentation/*.controller.ts`, `backend/src/modules/catalog/application/dto/*.ts` per Constitution IX (missing)
- [ ] T061 Convert the mocked "integration" specs to real Prisma/MySQL-backed tests using the project's MySQL service so idempotency and sync-job persistence are verified against actual database constraints instead of in-memory fakes — `backend/test/integration/catalog/sync-idempotency.spec.ts`, `backend/test/integration/catalog/sync-job.spec.ts` per plan: Testing, Constitution X (partial)
- [x] T062 Replace the floating-point `Math.round(Number(rate) * 100)` provider-cost normalization in `CurationService` with a deterministic decimal-to-minor-units parser that also accepts the numeric `rate` type already returned by `BulkFollowsClient`, avoiding floating-point monetary arithmetic — `backend/src/modules/catalog/application/curation.service.ts` per Constitution VIII (contradicts)
- [x] T063 Add request timeout and safe cancellation behavior to `BulkFollowsClient` provider calls, ensuring the API key is never exposed in errors, exceptions, or logs — `backend/src/modules/catalog/infrastructure/bulkfollows.client.ts` per Constitution VII, Constitution: Observabilidad y manejo de fallos (missing)
- [ ] T064 Ensure `MasterCatalogSnapshot.createdBy` and all audit actor identifiers are derived from the authenticated principal resolved by the authorization mechanism, never from a client-provided request body field — `backend/src/modules/catalog/snapshot/snapshot.service.ts`, `backend/src/modules/catalog/presentation/snapshot.controller.ts` per Constitution XI, FR-009 (contradicts)
- [ ] T065 Enforce the approved `Category` and `ProviderService` provenance references through Prisma relations and database-level foreign key/uniqueness constraints, including a reviewable forward-only migration — `backend/prisma/schema.prisma`, `backend/prisma/migrations/` per data-model.md, Constitution: Persistencia y migraciones (partial)