...
- [ ] T020 [US1] Implement provider import orchestration using `ProviderCatalogClient`: fetch provider services, map responses, upsert `ProviderService`, create or update staging records, and persist the final `SyncJob` summary — `backend/src/modules/catalog/sync/import-orchestrator.ts`
- [ ] T021 [US1] Implement partial-failure handling so provider request or mapping failures produce accurate `SyncJob` status, failure counts, and error summaries without falsely reporting success — `backend/src/modules/catalog/sync/import-orchestrator.ts`
- [ ] T022 [US1] Implement the curation application service for staged-service listing, approval, rejection, curated field edits, internal identifier generation, and provenance assignment — `backend/src/modules/catalog/application/curation.service.ts`
...
- [ ] T025 [US1] Implement protected administrative endpoints for listing staged services and approving or rejecting them with curated edits — `backend/src/modules/catalog/presentation/staged-service.controller.ts`
...
- [ ] T028 [US1] Add unit tests for staged-service approval, rejection, validation, curated field mapping, and audit creation — `backend/test/unit/catalog/curation.service.spec.ts`
...
- [ ] T031 [US2] Implement the `Category` domain model and category business invariants — `backend/src/modules/catalog/domain/category.entity.ts`
- [ ] T032 [US2] Implement the Prisma-backed `CategoryRepository` — `backend/src/modules/catalog/infrastructure/category.repository.ts`
- [ ] T033 [US2] Implement `CategoryService` for category creation, updates, listing, and allowed deletion or deactivation behavior — `backend/src/modules/catalog/application/category.service.ts`
- [ ] T034 [US2] Implement protected administrative category CRUD endpoints — `backend/src/modules/catalog/presentation/category.controller.ts`
...
- [ ] T038 [US2] Add unit tests for category business logic and CRUD behavior — `backend/test/unit/catalog/category.service.spec.ts`
