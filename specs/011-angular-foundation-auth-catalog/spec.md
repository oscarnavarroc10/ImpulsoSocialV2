...
**Fecha**: 2026-09-17  
**Estado**: Rediseño Aurora y autenticación en validación; catálogo real pendiente
...
2. cuatro presets visuales white-label y modo claro/oscuro/system configurable;
...
El contrato visual y de confiabilidad normativo se encuentra en
`design-contract.md`. No se permite sustituir sus decisiones concretas por la
preferencia estética del agente.
...
La compra, wallet, historial de órdenes y depósitos se implementarán sobre esta
base. Los enlaces correspondientes sólo pueden verse deshabilitados, sin
promesas temporales ni acciones simuladas.
...
- **Comercio antes que editorial**: el hero, las tarjetas y las acciones deben
  ayudar a descubrir servicios; no usar composición de revista, títulos
  gigantes ni decoración sin función.
- **Aurora comercial de forma predeterminada**: ImpulsoSocial inicia en modo
  claro con superficies blancas, texto grafito, verde de marca y un degradado
  azul-verde controlado únicamente en hero, bienvenida de cuenta y lienzo de
  autenticación. `dark` y `system` siguen disponibles como preferencias.
- **Recuperación visible**: un fallo esperado cambia a un estado de error
  comprensible y recuperable; nunca deja una pantalla blanca.
...
| `/login`         | Inicio de sesión canónico                   |
| `/registro`      | Registro canónico                           |
| `/cuenta`        | Resumen protegido de la cuenta              |
| `/auth/login`    | Alias compatible que redirige a `/login`    |
| `/auth/register` | Alias compatible que redirige a `/registro` |

Una ruta desconocida muestra una página 404 con CTA de regreso, sin redirigir
silenciosamente.

Mientras T008 esté pendiente, `/services` debe resolver a una vista honesta de
funcionalidad en preparación. Login y registro sí consumen los contratos reales
del backend. El resumen `/cuenta` muestra exclusivamente datos reales de sesión
y deja órdenes, wallet y seguimiento deshabilitados sin prometer una fecha. El
wildcard `**` permanece siempre al final del shell público.
...
Los métodos PayPal, Mercado Pago y tarjeta se muestran deshabilitados mientras
no exista integración backend, sin badges temporales. Transferencia y
...
- `icon` es una clave perteneciente a la allow-list de SVG locales definida en
  `design-contract.md`; nunca contiene emoji, HTML o SVG suministrado por JSON.
...
- `returnUrl` acepta únicamente rutas internas que comienzan con `/` y no con
  `//`; cualquier otra entrada se reemplaza por `/cuenta`;
...
- Google y Apple aparecen únicamente como preparación visual deshabilitada,
  gobernada por configuración y sin badges temporales; no navegan, no
  fabrican sesión y no llaman endpoints inexistentes.

### 5.6 Tema claro y oscuro

- opciones: `light`, `dark`, `system`;
- predeterminado inicial de ImpulsoSocial: `light`;
- presets iniciales: `spotify`, `minimal`, `x`, `instagram`;
- preset predeterminado: `spotify`;
- el preset y el modo se eligen y persisten independientemente;
- el valor predeterminado continúa siendo configurable por tenant;
...
interface TenantUiConfig {
  ...
  socialAuth: {
    google: { availability: "comingSoon" | "disabled" };
    apple: { availability: "comingSoon" | "disabled" };
  };
  catalog: {
...
ImpulsoSocial usa inicialmente la variante clara Aurora documentada en
`design-contract.md`: superficies blancas, texto grafito, verde inspirado en
Spotify y un degradado semántico controlado. La configuración expone variantes
Minimal, X e Instagram con paletas claras y oscuras completas. No se permiten
degradados en botones, textos, formularios o tarjetas ordinarias; tampoco neón o
glow. Magenta/naranja sólo aparecen dentro del preset Instagram. Si no hay logo
gráfico, se usa un wordmark accesible con el nombre configurado.
...
El arranque completo define `loading`, `ready` y `configurationError`. Todos los
`inject()` de un initializer deben ejecutarse antes del primer `await`. Carga de
configuración, tema, locale y persistencia deben ser idempotentes y tolerar
storage no disponible, preferencias corruptas y reintentos. El comportamiento
detallado se rige por `design-contract.md`.
...
- Componentes usan el comportamiento OnPush predeterminado de Angular 22; no se
  configura explícitamente `ChangeDetectionStrategy.OnPush`.
...
- OAuth real con Google o Apple; en esta feature sólo existe preparación visual
  deshabilitada y configurable;
...
- CA-003: tema claro/oscuro/system y los cuatro presets cambian sin recarga,
  persisten por separado y fallan de forma segura ante valores obsoletos.
...
- CA-014: métodos no integrados no son accionables ni muestran promesas
  temporales.
...
- CA-017: la aplicación arranca con el initializer real sin `NG0203`, promesas
  rechazadas sin manejar ni errores de consola.
- CA-018: config inexistente/inválida presenta una vista traducida con reintento;
  un reintento exitoso entra a la tienda sin recargar la página.
- CA-019: `es-MX.json` y `en.json` tienen paridad recursiva de rutas, tipos y
  longitudes de arrays; no hay texto visible o accesible hardcodeado.
- CA-020: ningún archivo de página/layout contiene estilos locales o inline; el
  sistema visual vive en `src/styles/**`.
- CA-021: el tema inicial es claro con preset Verde/Aurora; Minimal, X e
  Instagram aplican tokens completos sin recarga. Sólo hero, bienvenida de
  cuenta y lienzo de autenticación admiten el degradado construido con
  `heroStart`/`heroEnd`; no aparecen negro puro, neón, glow, gradientes en texto
  o botones, ni tipografía serif.
- CA-022: los iconos principales son SVG locales mediante claves allow-listed;
  no se usan emoji o caracteres Unicode como iconografía.
- CA-023: T001–T005 cuentan con al menos 25 pruebas focalizadas y validación
  manual en 320×568, 768×1024 y 1440×900 antes de pasar a T006.
- CA-024: toda operación asíncrona esperada termina en estado explícito y ningún
  método público deja estado parcial o listeners abandonados al fallar.
